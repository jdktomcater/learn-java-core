 ...
 * capabilities.
 *
 * 一个可重入共有排外锁，和方法与代码块的synchronized隐藏实现监控锁有着相同的行为与语义，
 *  * 但是它具有可扩展的能力。
 *
 *
 ...
 * #getHoldCount}.
 *
 * 一个可重入锁被最近获取锁还未释放锁的线程所拥有。当一个锁未被没有被其它线程所占有的话，
 * 一个线程调用他的lock方法时会成功返回并获取占用该锁，如果该线程已经占有该线程的话，该
 * 方法会立即返回。检测方法：isHeldByCurrentThread 与 getHoldCount
 *
